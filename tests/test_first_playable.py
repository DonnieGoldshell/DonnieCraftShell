...
class FirstPlayableTests(unittest.TestCase):
    ...
    def test_first_playable_launcher_live_economy_is_explicit_and_observable(self):
        ...
        self.assertIn('$env:DCS_LIVE_ECONOMY_PROVIDER_ORDER = "poe.show,poe.ninja"', start_script)
        self.assertIn("Live economy: ENABLED", start_script)
        self.assertIn("provider order: $liveEconomyProviderOrder", start_script)
        self.assertIn("poe.show -> poe.ninja", start_script)

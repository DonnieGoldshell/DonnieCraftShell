...
## Task 8C Modifier Pools

`ModifierPoolResolver` provides legal Exalted-style candidate filtering. It uses item class, affix side, required item level, open side, action-specific minimum modifier level, and source-backed modifier group conflicts. See [QUIVER_MODIFIER_POOL_STATUS.md](QUIVER_MODIFIER_POOL_STATUS.md).

Task 8C expands the selected PoE2DB Quiver natural Base pool to `7` Prefix families, `9` Suffix families, and `100` tier definitions. Natural Base Prefix and Suffix pools are `COMPLETE` only for that scoped source snapshot.

Probability completeness remains `UNKNOWN` because weights are unavailable and tier counts are not probabilities.

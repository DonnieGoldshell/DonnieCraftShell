...
- Issue #97 narrows real-trial capture into a guided preview/confirm workflow: before/after item states are parsed by the backend, the diff is proposed only when it matches one trusted outcome, operator confirmation is required before classification, ambiguous trials are saved only as unclassified, and duplicate records cannot inflate later empirical counts.
- Task 16B adds an observation review and curation gate. Recorder exports load as pending, users explicitly accept/reject/leave pending observations, accepted exports preserve original raw records for Task 15C, and a separate manifest audits rejected/pending/duplicate/context-warning decisions. Curation still does not prove crafting mechanics or numeric probabilities.
- Task 16C adds a curated import/build workflow from accepted review export to Task 15C aggregation and Task 15A-compatible raw empirical datasets. Dataset build summaries show counts, invalid records, duplicates, unclassified observations, warnings, and dataset IDs, but built datasets must still be explicitly configured/selected before they can affect Advisor probability readiness.
...
- Task 26 proves the First Playable Primed Quiver can become recommendation-ready only when current valuation, local material cost, explicit compatible probability evidence, and every required outcome valuation are supplied through existing contracts. See [FIRST_PLAYABLE_RECOMMENDATION_READINESS.md](FIRST_PLAYABLE_RECOMMENDATION_READINESS.md).
- Task 27 adds a Production Evidence Pilot workflow to the web workbench so Dennis can progress through the same real-evidence chain manually, distinguish saved local evidence from request-scoped evidence, send explicit bankroll/risk context, and rerun analysis without frontend recommendation logic. See [PRODUCTION_EVIDENCE_PILOT.md](PRODUCTION_EVIDENCE_PILOT.md).

...
- Task 18A adds `observation_workspace.py`: `ObservationWorkspaceRepository`, `FileBackedObservationWorkspaceRepository`, workspace entries, save results, and persistence status. These persist raw recorder records and separate review decisions locally, reject conflicting `raw_record_id` content, skip corrupt/version-incompatible storage conservatively, and export accepted records only through the existing review/import gates. See [OBSERVATION_WORKSPACE.md](OBSERVATION_WORKSPACE.md).
...
Valuation evidence must preserve listing/source identity and keep listing-derived estimates separate from realized sale prices. Current parsed items and hypothetical outcome states should map into the same valuation subject abstraction. Missing comparable currency conversion is unavailable, never zero.

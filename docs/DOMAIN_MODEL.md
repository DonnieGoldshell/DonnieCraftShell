...
- Task 17A extends `empirical_probability.py` with `EmpiricalProbabilityDatasetRegistry`, registration results, and dataset summaries. The registry stores loaded empirical datasets by ID, rejects conflicting duplicate IDs, and never activates a dataset unless Advisor analysis explicitly selects its ID. See [EMPIRICAL_DATASET_REGISTRY.md](EMPIRICAL_DATASET_REGISTRY.md).

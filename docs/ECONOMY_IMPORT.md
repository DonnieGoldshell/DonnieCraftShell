...
Task 6B implements the first offline PoE2 Economy Engine import path using a bounded poe.show Currency response.
...
`EconomyRepository` loads normalized snapshots in memory. `get_current_quote()` means latest available quote at or before the requested `as_of` time for the explicit league and asset. Stale quotes are returned with freshness metadata instead of being silently dropped.
